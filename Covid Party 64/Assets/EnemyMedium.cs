...
public class EnemyMedium : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Mathf.Abs(0/*target.position.x*/ - transform.position.x) < float.Epsilon)

            //If the y coordinate of the target's (player) position is greater than the y coordinate of this enemy's position set y direction 1 (to move up). If not, set it to -1 (to move down).
            enemyDir.y = /*target.position.y*/0 > transform.position.y ? 1 : -1;

        //If the difference in positions is not approximately zero (Epsilon) do the following:
        else
            //Check if target x position is greater than enemy's x position, if so set x direction to 1 (move right), if not set to -1 (move left).
            enemyDir.x = /*target.position.x*/ 0 > transform.position.x ? 1 : -1;
